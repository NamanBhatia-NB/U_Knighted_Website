...
export default function HeroSection() {
  return (
    <section className="relative min-h-screen flex flex-col md:flex-col lg:flex-row w-full items-center overflow-hidden py-4 md:py-6 lg:py-8" style={{ minHeight: '600px' }}>
      {/* Main content container - stacked on mobile, side by side on desktop */}
      <div className="container w-full px-4 sm:px-6 flex flex-col lg:flex-row items-center lg:items-start justify-between gap-8 relative z-10">
        ...
        <div className="w-full lg:w-1/2 max-w-3xl mx-auto lg:mx-0 order-2 lg:order-1 mt-6 lg:mt-0">
        ...
        {/* Chess board visualization - sized appropriately for each screen */}
        <div className="w-full sm:w-4/5 md:w-3/4 lg:w-1/2 h-[300px] sm:h-[350px] md:h-[400px] lg:h-[500px] xl:h-[550px] order-1 lg:order-2">
          <ChessBoard3D />
        </div>
      </div>
      
      {/* Decorative elements */}
      <div className="absolute bottom-0 left-0 right-0 h-24 sm:h-32 bg-gradient-to-t from-background to-transparent z-[5]"></div>
      <div className="absolute -bottom-5 left-1/2 transform -translate-x-1/2 z-[6] hidden sm:block">
